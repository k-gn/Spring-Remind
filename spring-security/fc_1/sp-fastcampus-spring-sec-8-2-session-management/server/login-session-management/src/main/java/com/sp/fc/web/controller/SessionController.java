...
public class SessionController {
    ...
    public String sessions(Model model){
        model.addAttribute("sessionList",
                sessionRegistry.getAllPrincipals().stream().map(p->UserSession.builder()
                        .username(((SpUser)p).getUsername()) // userdetails = principal
                        .sessions(sessionRegistry.getAllSessions(p, false).stream().map(s-> // 살아있는 세션만 가져오기
                                SessionInfo.builder()
    ...
    public String expireSession(@RequestParam String sessionId){
        ...
        if(!sessionInformation.isExpired()){
            sessionInformation.expireNow(); // 만료처리
        }
